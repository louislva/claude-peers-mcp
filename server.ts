...
const BROKER_SCRIPT = `${import.meta.dir}/broker.ts`;
...
async function loadToken(): Promise<void> {
  try {
    ...
    log("Warning: could not read broker token from " + TOKEN_PATH);
  }
}
...
async function main() {
  ...
  // 1.5. Load broker auth token (generated by broker on startup)
  await loadToken();
